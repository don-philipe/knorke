...
local tooltip
local state = {}
local current_day_format = '<span color="#1793D1"><u>%s</u></span>'

local function display_month(month,year,weekStart)
...
local function switch_month(delta)
	state[1] = state[1] + (delta or 1)
	local text = string.format('<span color="white" font_desc="monospace">%s</span>', display_month(state[1], state[2], 2))
...
function cal.register(mywidget, custom_current_day_format)
	...
	if not tooltip then
        ...
        function tooltip:update()
            ...
            state = {month, year}
            tooltip:set_markup(string.format('<span color="white" font_desc="monospace">%s</span>', display_month(month, year, 2)))
        end
        tooltip:update()
	end
